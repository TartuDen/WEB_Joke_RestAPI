...
app.get("/jokes/:index", (req, res) => {
    ...
    if (index >= 0 && index < jokes.length) {
        // Send the joke at the specified index
        res.status(200).json(jokes[index - 1]);
    } else {
...
app.get("/filter", (req, res) => {
    ...
    console.log(type)
    let selected = jokes.filter(joke => joke.jokeType === type);
    if (selected) {
        res.status(200).json({ jokes: selected })
    }
})
...
/**
 * Route to update a joke by index.
 * @param {Object} req - The request object.
 * @param {Object} res - The response object.
 */
app.put("/jokes/:index", (req, res) => {
    ...
    const indexToReplace = jokes.findIndex(joke => joke.id === idx);
    let newObject = {
        id: idx,
        jokeText: jokeText,
        jokeType: jokeType
    ...
    if (indexToReplace !== -1) {
        jokes.splice(indexToReplace, 1, newObject);
    } else {
        res.status(404).json({ error: "index was not found" })
    }
    res.status(200).json(newObject);
})

/**
 * Route to partially update a joke by id.
 * @param {Object} req - The request object.
 * @param {Object} res - The response object.
 */
app.patch("/jokes/:id", (req, res) => {
    let idx = parseInt(req.params.id);
    let { jokeText, jokeType } = req.body;
    let idxToUpdate = jokes.findIndex(joke => joke.id === idx);
    if (idxToUpdate !== -1) {
        // Update jokeText if provided in the request body
        if (jokeText !== undefined) {
            jokes[idxToUpdate].jokeText = jokeText;
        }

        // Update jokeType if provided in the request body
        if (jokeType !== undefined) {
            jokes[idxToUpdate].jokeType = jokeType;
        }

    } else {
        res.status(404).json({ error: "joke not found" });
    }
    res.status(200).json(jokes[idxToUpdate]);
